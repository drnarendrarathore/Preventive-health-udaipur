    ...
    return (
        <div className={`step-content reader-view ${themeClass}`}>
            ...
            <div className="reader-hero">
                <span className="reader-category">{selectedRec.category}</span>
                <h1 className="reader-title">{selectedRec.test}</h1>
                <span className="reader-meta">{selectedRec.frequency}</span>
            </div>
            <div className="reader-body">
                {details.map((detail, index) => {
                    if ('title' in detail) {
                        return (
                            <section key={index} className="reader-section animate-fade-in" style={{ animationDelay: `${index * 100}ms` }}>
                                <h4>{detail.title}</h4>
                                <p>{detail.content}</p>
                            </section>
                        );
            ...
            <div className="step-actions">
                 <button onClick={handleCloseDetail} className="btn btn-primary" style={{ width: '100%' }}>
                    {t('close_modal')}
                 </button>
            </div>
        </div>
