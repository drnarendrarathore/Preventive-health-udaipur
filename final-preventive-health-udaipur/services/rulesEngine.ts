...
function calculateCBACScore(patient: UserData): number {
    ...
    else if (patient.age >= 30 && patient.age <= CLINICAL_THRESHOLDS.CBAC_AGE_BRACKETS.LOW) score += 1;
    else if (patient.age >= 40 && patient.age <= CLINICAL_THRESHOLDS.CBAC_AGE_BRACKETS.MEDIUM) score += 2;
    else if (patient.age >= 50 && patient.age <= CLINICAL_THRESHOLDS.CBAC_AGE_BRACKETS.HIGH) score += 3;
    else score += 4;
    ...
    if (isMale) {
        if (patient.waistCircumference > CLINICAL_THRESHOLDS.WAIST_MALE_HIGH) score += 2;
    } else { // Stricter threshold for female and 'other' genders
        if (patient.waistCircumference > CLINICAL_THRESHOLDS.WAIST_FEMALE_HIGH) score += 2;
    }
...
export function generateRecommendations(patient: UserData): AnalysisResponse {
  ...
  const packYears = (patient.smokingStatus !== 'never') 
    ? ((patient.smokingPacksPerDay || 0) * (patient.smokingYears || 0)) 
    : 0;
  ...
  const meetsLungCriteria = patient.age >= CLINICAL_THRESHOLDS.LUNG_SCREENING_MIN_AGE && patient.age <= CLINICAL_THRESHOLDS.LUNG_SCREENING_MAX_AGE && packYears >= CLINICAL_THRESHOLDS.LUNG_SCREENING_MIN_PACK_YEARS;
  ...
  if (patient.gender === "female" && patient.age >= CLINICAL_THRESHOLDS.CERVICAL_SCREENING_MIN_AGE && patient.age <= CLINICAL_THRESHOLDS.CERVICAL_SCREENING_MAX_AGE) {
  ...
  if (patient.gender === "female") {
    ...
    const isHighRiskBreast = patient.ashkenaziAncestry || hasHighRiskFamilyHistory;
    const isEligibleForMammogram = (patient.age >= CLINICAL_THRESHOLDS.BREAST_SCREENING_MIN_AGE && patient.age <= CLINICAL_THRESHOLDS.BREAST_SCREENING_MAX_AGE) || (isHighRiskBreast && patient.age >= CLINICAL_THRESHOLDS.BREAST_SCREENING_HIGH_RISK_MIN_AGE && patient.age <= CLINICAL_THRESHOLDS.BREAST_SCREENING_MAX_AGE);
  ...
  // --- PROSTATE CANCER ---
  if (patient.gender === 'male' && patient.age >= CLINICAL_THRESHOLDS.PROSTATE_SCREENING_HIGH_RISK_MIN_AGE) {
      ...
      if (patient.age >= CLINICAL_THRESHOLDS.PROSTATE_SCREENING_MIN_AGE || hasHighRiskProstate) {
          let reasonKey = 'prostate_reason';
          if(hasHighRiskProstate) {
              reasonKey = 'prostate_high_risk_reason';
              highRiskFlags.add('prostate');
          }
          if(hasEarlyOnsetProstate) { // Early onset is always high priority
             highRiskFlags.add('prostate');
          }
  ...
  const isEligibleForColonScreening = (patient.age >= CLINICAL_THRESHOLDS.COLON_SCREENING_MIN_AGE && patient.age <= CLINICAL_THRESHOLDS.COLON_SCREENING_MAX_AGE) || (isHighRiskColon && patient.age >= CLINICAL_THRESHOLDS.COLON_SCREENING_HIGH_RISK_MIN_AGE && patient.age <= CLINICAL_THRESHOLDS.COLON_SCREENING_MAX_AGE);
  ...
  if (patient.saltIntake === 'high' && patient.age >= 35) {
      recs.set('gastric_screening', 'gastric_screening_reason');
  }

  if (patient.hepatitisHistory !== 'none') recs.set('liver_hep', 'liver_hep_reason');
  if (patient.hpvVaccineStatus !== 'complete' && patient.age <= 45) recs.set('hpv_prevention', 'hpv_prevention_reason');
